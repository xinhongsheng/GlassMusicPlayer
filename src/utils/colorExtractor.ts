...
const resolveImageUrl = (imageUrl: string): string => {
  ...
  if (mediaProxyPrefix && imageUrl.startsWith(mediaProxyPrefix)) return imageUrl
  ...
  if (!mediaHostRe.test(url.hostname)) return imageUrl

  const httpsUrl = imageUrl.replace(/^http:\/\//i, 'https://')
  if (!mediaProxyPrefix) return httpsUrl
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      try {
        ...
        // 更激进的采样 (每 20 个像素采样一次)
        const pixels: RGB[] = []
        for (let i = 0; i < data.length; i += 80) { // 每 20 个像素
          const r = data[i]
          const g = data[i + 1]
          const b = data[i + 2]
          const a = data[i + 3]
        ...
        const enhancedVibrant = enhanceSaturation(vibrantColor.r, vibrantColor.g, vibrantColor.b, 1.3)

        // 柔和色: 降低饱和度
        const mutedColor = sortedByVibrance[sortedByVibrance.length - 1]
        const enhancedMuted = adjustBrightness(mutedColor.r, mutedColor.g, mutedColor.b, 1.2)
    ...
    // 添加时间戳避免缓存问题
    img.src = resolvedUrl.includes('?') ? `${resolvedUrl}&t=${Date.now()}` : `${resolvedUrl}?t=${Date.now()}`
  })
}
...
export async function getColorPalette(imageUrl: string, useCache: boolean = true): Promise<ColorPalette> {
